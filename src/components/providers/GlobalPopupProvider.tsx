      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            transition={{ duration: 0.2, ease: "easeOut" }}
            style={{ zIndex: 10000 }}
            className="bg-theme-card text-left p-6 rounded-[28px] shadow-2xl max-w-[360px] w-full relative overflow-hidden border border-theme-subtle flex flex-col gap-4"
          >
            {/* 상단 은은한 그린 앰비언트 글로우 */}
            ...
            <div className="flex flex-col gap-1 relative z-10">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-extrabold txt-brand-ink tracking-tight leading-snug">
                  {params.title}
                </h2>
                <PencilSimple size={18} className="text-[#00C474] shrink-0 ml-2" weight="bold" />
              </div>
              {params.subtitle && (
                <p className="txt-caption-main text-theme-muted font-medium leading-relaxed">
                  {params.subtitle}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5 relative z-10 mt-1">
              {params.inputLabel && (
                <label className="txt-caption-main text-theme-muted font-medium mb-0.5 block">
                  {params.inputLabel}
                </label>
              )}
              <div className="flex items-center gap-2.5 bg-theme-card-subtle border border-theme-subtle rounded-2xl px-4 py-3.5 focus-within:border-[#00C474] focus-within:ring-2 focus-within:ring-[#00C474]/20 transition-all duration-200">
                <input
                  type="text"
                  value={inputValue}
            ...
            <div className="grid grid-cols-2 gap-2.5 pt-2 relative z-10">
              <button
                type="button"
                onClick={closePopup}
                className="py-3.5 px-4 rounded-2xl text-base font-extrabold text-theme-muted bg-theme-card-subtle hover:brightness-95 transition-all text-center"
              >
                취소
              </button>
              <button
                ...
                className="py-3.5 px-4 rounded-2xl text-base font-extrabold text-white bg-[#00C474] hover:bg-emerald-600 shadow-md transition-all flex items-center justify-center gap-1.5"
              >
                <span>저장하기</span>
