...
/**
 * 1. ShowcaseHeader: 상단 앱 메인 메뉴 헤더 ([BrandLogo (좌측, 클릭 불가)] + [알림 종 버튼 (우측)])
 */
export function ShowcaseHeader({
  ...
  return (
    <div className="w-full flex items-center justify-between py-1 px-0.5">
      ...
      {/* Right Mobile Status (우측 알림 종 버튼 -> /notifications) */}
      <div className="flex items-center">
        <button
          type="button"
          aria-label="알림 수신함"
          onClick={() => router.push("/notifications")}
          className="p-1.5 txt-brand-ink hover:text-gray-900 rounded-full hover:bg-gray-100 transition-colors relative active:scale-95 flex items-center justify-center outline-none"
        >
